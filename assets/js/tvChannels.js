...
function tvButtonTouchstart(event) {
  event.target.children[0].style.visibility = "visible";
}

function tvButtonTouchend(event) {
  event.target.children[0].style.visibility = "hidden";
}

tvChannelDiv.addEventListener("touchstart", tvButtonTouchstart);
tvChannelDiv.addEventListener("touchend", tvButtonTouchend);
...
document.querySelector("#fetch-network-navigation-button-next").addEventListener("touchstart", function (event) {
  ...
  currentNetwork.innerHTML = all_networks[network_chosen];
  var div = document.getElementById("fetch-channel-div");
...
document.querySelector("#fetch-network-navigation-button-previous").addEventListener("touchstart", function (event) {
  if (network_chosen == 0) {
...
document.querySelector("#fetch-remote-style").addEventListener("touchstart", function (event) {
  fetchbuttonflip.classList.toggle("dpad-active");
  fetchbuttonflip.classList.toggle("numpad-active");
  fetchDpadCasing.classList.toggle("display-none-class")
  fetchNumpadCasing.classList.toggle("display-none-class")
});


document.querySelector("#foxtel-remote-style").addEventListener("touchstart", function (event) {
  foxtelbuttonflip.classList.toggle("dpad-active");
  foxtelbuttonflip.classList.toggle("numpad-active");
  foxtelDpadCasing.classList.toggle("display-none-class")
  foxtelNumpadCasing.classList.toggle("display-none-class")
});
